...
{
    public partial class CutQtyForm : MaterialForm
    {
        string se_id = null;
        string size_no = null;

        public CutQtyForm(string vSeId, string vSizeNo)
        {
            InitializeComponent();
            se_id = vSeId;
            size_no = vSizeNo;
        ...
        private void CutQtyForm_Load(object sender, EventArgs e)
        {
            SJeMES_Framework.Common.UIHelper.UIUpdate(this.Name, this, Program.client, "", Program.client.Language);
            GetCutQtyDetail(se_id, size_no);
           
        }

        private void GetCutQtyDetail(string se_id, string size_no)
        {
